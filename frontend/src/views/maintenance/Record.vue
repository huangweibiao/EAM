  ...
  <div class="maintenance-record">
    <el-card>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        <el-table-column prop="maintenanceCode" label="维护单号" width="150" />
        ...
        <el-table-column prop="result" label="结果" width="100">
          <template #default="{ row }">
            <el-tag :type="row.result === 'COMPLETED' ? 'success' : 'warning'">{{ getResultText(row.result) }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="维护日期">
              <el-date-picker v-model="formData.maintenanceDate" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
const getResultText = (result: string) => {
  const map: Record<string, string> = { 'COMPLETED': '已完成', 'PENDING': '待处理', 'FAILED': '失败' }
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await maintenanceApi.recordPage({
...
const handleEdit = (row: any) => {
  ...
  formData.cost = row.cost || 0
  formData.technician = row.technician || ''
  formData.result = row.result || 'COMPLETED'
...
const handleSubmit = async () => {
  ...
  try {
    if (formData.id) {
      await maintenanceApi.recordUpdate(formData)
      ElMessage.success('修改成功')
    } else {
      await maintenanceApi.recordAdd(formData)
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('确认删除该记录吗？', '提示', { type: 'warning' }).then(async () => {
    try {
      await maintenanceApi.recordDelete(row.id)
