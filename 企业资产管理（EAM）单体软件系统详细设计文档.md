...
|create_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP|创建时间|
|update_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP ON UPDATE|更新时间|
#### 4. 资产变动记录表 (eam_asset_change_log)
...
|create_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP|创建时间|
|update_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP ON UPDATE|更新时间|
#### 8. 维护执行记录表 (eam_maintenance_record)
...
|create_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP|创建时间|
|update_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP ON UPDATE|更新时间|
#### 10. 备件分类表 (eam_spare_part_category)
...
|create_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP|创建时间|
|update_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP ON UPDATE|更新时间|
#### 14. 工单-备件关联表 (eam_work_order_part)
...
|create_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP|创建时间|
|update_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP ON UPDATE|更新时间|
#### 18. 资产盘点明细表 (eam_asset_inventory_detail)
...
|create_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP|创建时间|
|update_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP ON UPDATE|更新时间|
#### 22. 角色-权限关联表 (eam_sys_role_permission)
...
|create_time|datetime|-|NOT NULL|CURRENT_TIMESTAMP|创建时间|
#### 23. 操作日志表 (eam_sys_operation_log)
...
|表名|索引字段|索引类型|说明|
|---|---|---|---|
|eam_asset|asset_code|UNIQUE|资产编码唯一索引|
|eam_asset|dept_id|INDEX|按部门查询资产|
|eam_asset|category_id|INDEX|按分类查询资产|
|eam_asset|status|INDEX|按状态查询资产|
|eam_asset|next_maintenance_date|INDEX|维护提醒查询|
|eam_work_order|order_no|UNIQUE|工单编号唯一索引|
|eam_work_order|asset_id|INDEX|按资产查询工单|
|eam_work_order|status|INDEX|按状态查询工单|
|eam_work_order|assigned_to|INDEX|按负责人查询工单|
|eam_work_order|report_time|INDEX|按报修时间查询|
|eam_spare_part|part_code|UNIQUE|备件编码唯一索引|
|eam_spare_part|category_id|INDEX|按分类查询备件|
|eam_spare_part|quantity|INDEX|库存预警查询|
|eam_maintenance_record|asset_id|INDEX|按资产查询维护记录|
|eam_maintenance_record|maintenance_date|INDEX|按时间查询维护记录|
|eam_purchase_order|order_no|UNIQUE|采购订单号唯一索引|
|eam_purchase_request|request_no|UNIQUE|申请单号唯一索引|
|eam_sys_user|username|UNIQUE|用户名唯一索引|
|eam_sys_user|dept_id|INDEX|按部门查询用户|
|eam_asset_inventory|inventory_no|UNIQUE|盘点单号唯一索引|
|eam_asset_inventory_detail|inventory_id,asset_id|UNIQUE|盘点单+资产唯一索引|
## 五、关键业务规则
